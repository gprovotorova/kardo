...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public List<PostDto> getAllPostsByUserId(Long userId, Pageable page) {
        userRepository.existsById(userId);
        return postRepository.findAllByAuthor_Id(userId, page)
                .getContent()
                .stream()
    ...
    public List<CommentDto> getAllCommentsByPostId(Long postId, Pageable page) {
        Post post = postRepository.findById(postId)
                .orElseThrow(() -> new ObjectNotFoundException("Пост  с id = " + postId + " не найден."));
        ...
        return commentRepository.findAllByPostId(postId, page)
                .getContent()
                .stream()
                .map(comment -> CommentMapper.toCommentDto((Comment) comment, post, post.getAuthor()))
                .collect(Collectors.toList());
    }
    ...
    @Override
    @Transactional(readOnly = true)
    public List<CommentDto> getAllCommentsByUserId(Long userId, Pageable page) {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new ObjectNotFoundException("Пользователь с id = " + userId + " не найден."));
